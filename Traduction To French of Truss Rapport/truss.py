...
class MainPage(QWizardPage):
    ...
    def generate_report(self):
        ...
        # """))
        "old code"
        story.append(Paragraph("<font size='25'>Rapport d'analyse de la ferme</font>", styles['Title']))
        story.append(Spacer(1, 105))
        story.append(Paragraph(
        f"<para alignment=center size=12 color='steelblue'><strong>Nom du projet :</strong> {project}</para>"))
        story.append(Spacer(1, 3))
        story.append(Paragraph(
        f"<para alignment=center size=12 color='steelblue'><strong>Nom d'utilisateur :</strong> {username}</para>"))
        story.append(Spacer(1, 3))
        story.append(Paragraph(
        f"<para alignment=center size=12 color='steelblue'><strong>Créé le :</strong> {date}</para>"))
        story.append(Spacer(1, 100))
        story.append(Paragraph(
        "<font size='20'><b>Ce rapport a été généré en utilisant </b>-----</font>"))
        story.append(Spacer(1, 30))
        story.append(Paragraph(f"""<para color='green' alignment=center>
        {banner_1}
        </para>"""))
        ...
        story.append(Spacer(1, 100))
        story.append(Paragraph("""N.B: Cette application a été développée à des fins éducatives uniquement.
        Les étudiants ou les enseignants peuvent utiliser cette application librement."""))
        story.append(PageBreak())
       
           # Page 2 units
        story.append(Paragraph(f"""<font size='20' color='steelblue'> Unités : Système usuel américain d'unités </font><br/><br/>
        <b>Longueur:</b> {self.unit_report[0]}<br/>
        <b>Charge appliquée:</b> {self.unit_report[1]}<br/>
        <b>Forces de membre:</b> {self.unit_report[2]}<br/>
        <b>Module d'élasticité (E):</b> {self.unit_report[3]}<br/>
        <b>Aire de section transversale (A):</b> {self.unit_report[4]}<br/>
        <b>Déplacement :</b> {self.unit_report[-1]}<br/><br/><br/>
        <font size='20' color='steelblue'>Géométrie de la ferme : Nœuds </font><br/><br/>
        Nodes are the points in (x,y) co-ordinate.<br/><br/><br/>"""))
